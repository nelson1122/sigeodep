...
public class GeoDBConnection implements Serializable {
    ...
    public void refreshIndicatorData(int user_id, int indicator_id, ArrayList<Variable> order) {
        System.out.println(user_id + " - " + indicator_id);
    ...
    public List<MfFeature> getPolygons(RangeFactory rf) {
        ...
        String query = ""
                + "SELECT "
                + "	record_id, " + geo_column + ", count, geom "
                + "FROM "
                + "	indicators_records "
                + "INNER JOIN "
                + "	neighborhoods "
                + "ON "
                + "	neighborhood_name LIKE " + geo_column + " "
                + "WHERE "
                + "	geom IS NOT NULL "
        ...
        try {
            while (records.next()) {
                ...
                final String fname = records.getString(geo_column);
